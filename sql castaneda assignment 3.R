...
dbWriteTable(conn, "carriers", carriers_df)
dbWriteTable(conn, "planes", planes_df)
for(i in c(2000:2005)) {
  ontime_temp_df <- read.csv(paste0(i, ".csv"), header = TRUE)
  if(i == 2000) {
    dbWriteTable(conn, "ontime", ontime_temp_df)
  } else {
    dbWriteTable(conn, "ontime", ontime_temp_df, append = TRUE)
  }
}
q1 <- dbGetQuery(conn, 
...
q3 <- dbGetQuery(conn, 
 ...
 WHERE ontime.Cancelled = 1 AND 
       carriers.Description IN ('United Air Lines Inc.', 'American Airlines Inc.', 'Pinnacle Airlines Inc.', 'Delta Air Lines Inc.')
